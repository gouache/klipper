...
일반적으로 매개변수는 printf() 스타일 구문(예: "%u")으로 설명됩니다. 
...
첫 번째 예의 경우 DECL_ENUMERATION() 매크로는 매개변수 이름이 
...
호스트에서 마이크로 컨트롤러로 전송된 각 메시지 블록에는 내용에 일련의 
...
저수준 호스트 코드는 마이크로 컨트롤러로 전송된 손실 및 손상된 메시지 블록에 대한 
